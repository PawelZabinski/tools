...
<header class="bg-base-200 dark:bg-dark-base-200 bg-opacity-80 backdrop-blur-md border-b border-base-300 dark:border-dark-base-200 fixed w-screen text-gray-800 h-16 flex justify-between md:justify-around items-center p-5 z-50">
    <a href="/" class="dark:text-base-400 font-bold text-xl">tools.zabinski.dev</a>

    <section class="md:flex hidden dark:text-base-300">
        <a href="https://zabinski.dev" class="px-4 py-2 hover:bg-base-200 dark:hover:bg-dark-base-150 transition-colors duration-200 rounded-xl">Portifolio</a>
        <a href="mailto:developer@zabinski.dev" class="px-4 py-2 font-bold hover:bg-base-200 dark:hover:bg-dark-base-150 transition-colors duration-200 rounded-xl">Contact</a>
    </section>
...
{#if isShowingMenu}
    <div transition:fly={{ y: -100, duration: 200 }} class="fixed py-2 px-2 w-full z-50">
        <div class="block md:hidden w-full py-2 px-2 space-y-2 font-medium bg-base-200 dark:bg-dark-base-200 rounded-xl">
            <a href="https://zabinski.dev" class="rounded-lg block px-4 py-5 text-sm text-gray-800 dark:text-base-300 hover:bg-base-300 dark:hover:bg-dark-base-150">Portfolio</a>
...
<section class="w-screen min-h-screen flex flex-col justify-center items-center">
    <main class="p-3 py-3 lg:px-0 limit-screen-width">
        <section class="text-center animated-gradient border border-base-300 dark:border-dark-base-300 dark:text-base-400 rounded-2xl py-8">
            <h1 class="text-3xl sm:text-5xl p-3 font-bold">Specialised Tools</h1>
        ...
        <section class="w-full mx-auto grid gap-5 place-items-center grid-cols-1 md:grid-cols-2 lg:grid-cols-3 py-5 h-full">
            {#each orderedTools as tool (tool.id)}
                ...
                <div
                    ...
                    class="group relative h-36 text-left w-full bg-base-200 dark:bg-dark-base-200 dark:text-base-400 bg-opacity-60 backdrop-blur-lg hover:bg-opacity-80 border rounded-2xl"
                >
                    <a href={`/${tool.path}`} class="w-full h-full p-3 flex flex-col justify-center items-center space-y-2">
                        ...
                        <p class="text-slate-900 dark:text-base-200 w-full text-sm">{tool.description}</p>
                    </a>

                    <button on:click={() => addSwapIndex(tool.id)} class:hidden={!isSelected} class="absolute group-hover:block top-1 right-1 p-3 hover:bg-base-300 dark:hover:bg-dark-base-150 rounded-xl">
